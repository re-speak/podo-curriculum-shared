/* ================================================================
   FEEDBACK · 말한 문장 → 고친 문장 → 달라진 곳(diff)

   Core · Contextual · Freetalking 이 함께 쓰는 피드백 활동이다.
   activities.js 다음, pager.js 전에 로드한다.
   ================================================================ */
...
(function () {
  ...
  function cut(fb) {
    ...
    // 문장 만들기에서는 할 일과 어휘 힌트가 첫 교정에만 붙는다.
    // 뒤에 교정/노트를 더 열어도 원래 발화의 스캐폴드는 첫 카드와 함께 남는다.
    var first = fb.querySelector(".fb-block");
    if (first && task) first.insertBefore(task, first.firstChild);
    if (first && hint) first.appendChild(hint);
  }
